    ...
    describe(testFramework, function () {
      ...
      it('ember g feature foo', function () {
        ...
        return emberGenerateDestroy(args, (file) => {
          expect(file('tests/acceptance/foo.feature')).to.equal(
            fixture('acceptance/foo.feature')
          );
          expect(file('tests/acceptance/steps/foo-steps.js')).to.equal(
            fixture(`acceptance/${testFramework}/foo-steps.js`)
          );
        });
      });
      ...
      testTypes.forEach((type) => {
        it(`ember g feature foo --type=${type}`, function () {
          ...
          return emberGenerateDestroy(args, (file) => {
            expect(file(`tests/${type}/foo.feature`)).to.equal(
              fixture(`${type}/foo.feature`)
            );
            expect(file(`tests/${type}/steps/foo-steps.js`)).to.equal(
              fixture(`${type}/${testFramework}/foo-steps.js`)
            );
          });
        });
      });
